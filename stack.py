...
def make_Z(models):
	...
	cvpreds_list = list(map(lambda model: get_cvpreds(model), models))
...
def metapredict(models, metafit, test_data):
	...
	basepred.columns = list(map(lambda model: model.model_id, models)) # names of the models (design-matrix-for-metalearner names)
